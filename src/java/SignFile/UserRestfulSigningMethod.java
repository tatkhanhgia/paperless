 ...
 * @author GiaTK
 */
public class UserRestfulSigningMethod implements SigningMethodSync, SigningMethodAsync {
    ...
    // remove declare final to init constructor without parameter
    public UserRestfulSigningMethod() {
    }

    public UserRestfulSigningMethod(String _credentialID, String _pin, IUserSession _usersession, SignAlgo _signAlgo, HashAlgorithmOID _hashAlgo) {
    ...
    public List<String> getCert() throws Exception {
        ...
        try {
            iCert = _usersession.certificateInfo(_credentialID);
        } catch (Throwable ex) {
        }
    ...
    public List<String> sign(List<String> hashList) throws Exception {
        ...
        String sad = null;
        try {
            sad = _usersession.authorize(_credentialID, hashList.size(), "", _pin);
        } catch (Throwable ex) {
            Logger.getLogger(RestfulSigningMethod.class.getName()).log(Level.SEVERE, null, ex);
        }
        List<byte[]> signatures = null;
        try {
            signatures = _usersession.signHash(_credentialID, doc, _signAlgo, sad);
        } catch (Throwable ex) {
            Logger.getLogger(RestfulSigningMethod.class.getName()).log(Level.SEVERE, null, ex);
        }
